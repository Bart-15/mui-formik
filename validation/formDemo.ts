  ...
  commentAboutInvestmentRisk: mixed().when('investmentRisk', {
    ...
    then: (_schema) =>
      string()
        ...
        .max(
          100,
          'Comment About Investment Risk must be at most 100 characters'
        ),
    otherwise: (schema) => schema.notRequired(),
  }),
  dependents: number()
    .required()
    .min(0, 'Dependents must be greater than or equal 0')
    .max(5),
  acceptedTermsAndConditions: boolean().oneOf(
    [true],
    'You must agree to accept the terms and conditions'
  ),
});
