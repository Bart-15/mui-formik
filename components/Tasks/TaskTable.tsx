    ...
    return ( 
        <TableContainer component={Paper}>
                <Table sx={{ minWidth: 650 }} aria-label="task table">
                    ...
                    <TableBody>
                    { tasks.length === 0 && (<TableRow><TableCell colSpan={6}>No data available</TableCell></TableRow>) }
                        {
                            tasks.map((task) => (
                                <TableRow key={task.id}>
                                    <TableCell align="left" component="th" scope='row'>{task.id}</TableCell>
                                    <TableCell align="left">{task.task}</TableCell>
                                    <TableCell align="left">{task.completed ? "Yes" : "No"}</TableCell>
                                    <TableCell align="left">
                                        <Button type="button" variant="outlined" color="error" onClick={() => deleteTask(task.id)}>Delete</Button>
                                    </TableCell>
                                </TableRow>
                            ))
                        }
